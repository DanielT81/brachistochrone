...
amount_of_total_points = 15 # amount of total points in the system
precision_value = 190 # the precision value for the calculation
...
def calc_arr_time(def_name) -> np.ndarray:
	...
	for def_index in range(amount_of_total_points - 1):
		...
		def_vel = def_new_vel
	return def_time
def move_random_point(def_new_arr) -> None: # rng point in arr gets moved by amount and stored in same column in new_arr
	...
	def_new_arr[def_index] = def_new_point
def loop(def_arr, def_total_iterations, safeguard) -> np.ndarray: # loop that creates the new points and compares whether they are better that way
	waiting=0
	def_total_iterations = def_total_iterations
	def_arr = def_arr
	def_arr_time = calc_arr_time(def_arr)  # the (yet initial) time take for the points in arr
	...
	while waiting < safeguard:#sqr(ATP * 5): # Term is to make sure that there is no more optimizing possible
		...
		def_new_arr = np.array(def_arr, copy=True)
		move_random_point(def_new_arr)
		new_time = calc_arr_time(def_new_arr)
		#print(f'new_time: {new_time} \narr_time: {arr_time}')
		def_total_iterations += 1
		if new_time < def_arr_time:
			def_arr = def_new_arr
			def_arr_time = new_time
			print(waiting)
			waiting = 0
		else:
			def_new_arr = def_arr
	...
	print(f'the amount of total iterations is: {def_total_iterations}')
	print(f'the time for the optimized points is: {def_arr_time} second')
	print(np.array([def_arr, def_arr_time, waiting, def_total_iterations], dtype = object))
	return np.array([def_arr, def_arr_time, waiting, def_total_iterations], dtype = object)
